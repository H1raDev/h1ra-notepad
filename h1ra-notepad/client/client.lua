...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function() TriggerEvent("h1ra-notepad:Client:Sync") end)
AddEventHandler('onResourceStart', function(r) if GetCurrentResourceName() ~= r then return end TriggerEvent("h1ra-notepad:Client:Sync") end)

RegisterNetEvent("h1ra-notepad:Client:Sync", function(newNotes)
	if not newNotes then
		local p = promise.new()
		QBCore.Functions.TriggerCallback('h1ra-notepad:Server:Sync', function(cb) p:resolve(cb) end)
	...
	for k, v in pairs(Notes) do
		if not Props[k] and Notes[k] then
			...
			Targets[k] =
				...
				{ options = { { type = "server", event = "h1ra-notepad:Server:Read", icon = "fas fa-receipt", label = Loc[Config.Lan].targetinfo["read"], noteid = k, }, },
						distance = 1.5 })
		end
	end
...
RegisterNetEvent("h1ra-notepad:Client:Create", function()
    ...
    if dialog.note or dialog[1] then
		...
		Wait(900)
        TriggerServerEvent("h1ra-notepad:Server:Create", {
...
RegisterNetEvent("h1ra-notepad:Client:Read", function(data)
	local notepad = {}
	notepad[#notepad+1] = {
		icon = "fas fa-receipt",
		isMenuHeader = true,
	...
	notepad[#notepad+1] = {
		...
		event = "h1ra-notepad:Client:DestroyNote", args = data,
		params = { event = "h1ra-notepad:Client:DestroyNote", args = data }
	}
	if Config.Menu == "ox" then
...
RegisterNetEvent("h1ra-notepad:Client:DestroyNote", function(data)
	loadAnimDict("pickup_object")
	TaskPlayAnim(PlayerPedId(), "pickup_object", "pickup_low", 8.0, -8.0, -1, 0, 0, false, false, false)
	unloadAnimDict("pickup_object")
	Wait(900)
	TriggerServerEvent("h1ra-notepad:Server:SyncEffect", data.coords)
	TriggerServerEvent("h1ra-notepad:Server:DestroyNote", data.id)
end)

RegisterNetEvent("h1ra-notepad:Client:SyncEffect", function(coords)
